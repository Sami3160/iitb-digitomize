...
const {getGroqCompletion} =require('../config/ai.config')
...
exports.summarizeBlogContent = async (req, res) => {
  ...
  try {
    ...
    const comments =
      blog.comments.length > 0
        ? blog.comments.map((c) => `- ${c.content}`).join("\n")
        : "No comments yet.";
    ...
    const systemInstruction = `
      You are a content formatter for a mobile app. 
      Summarize the blog post and comments using ONLY valid HTML tags.
      ...
      - Wrap the entire response in a <div>.
      - DO NOT use <html>, <body>, or <style> tags.
      
      Structure:
      1. An <h3>Summary</h3> followed by a <p>.
      2. An <h3>Key Insights</h3> followed by a <ul> list.
      3. An <h3>Community Pulse</h3> analyzing the comments.
    `;

    const aiResponse = await getGroqCompletion(systemInstruction, aiInput);
...
exports.deleteComment = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Comment deleted successfully", updatedBlog });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: error.message });
  }
    ...
    if (user.blogs && user.blogs.length > 0) {
      user.blogs.forEach((blog) => {
        ...
        const diffInDays = Math.floor(
          (createdAt - startOfYear) / (1000 * 60 * 60 * 24)
        );
